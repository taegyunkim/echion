...
void ThreadInfo::update_cpu_time() {
  ...
  kern_return_t kr =
      thread_info((thread_act_t)this->mach_port, THREAD_BASIC_INFO,
  ...
  this->cpu_time =
      TV_TO_MICROSECOND(info.user_time) + TV_TO_MICROSECOND(info.system_time);
#endif
}
...
inline std::unordered_map<uintptr_t, ThreadInfo::Ptr> &thread_info_map =
    *(new std::unordered_map<uintptr_t,
                             ThreadInfo::Ptr>()); // indexed by thread_id
...
void ThreadInfo::unwind_tasks() {
  ...
  std::unordered_map<PyObject *, TaskInfo::Ref>
      waitee_map; // Indexed by task origin
  std::unordered_map<PyObject *, TaskInfo::Ref>
      origin_map; // Indexed by task origin
...

  {
    ...
    std::transform(
        task_link_map.cbegin(), task_link_map.cend(),
...
void ThreadInfo::unwind_greenlets(PyThreadState *tstate,
  ...
  for (auto &kv : greenlet_info_map) {
    std::cerr << "greenlet_info_map key: " << kv.first << " value: "
  ...
  std::transform(greenlet_parent_map.cbegin(), greenlet_parent_map.cend(),
  ...
  for (auto &greenlet_info : greenlet_info_map) {
    ...
    std::cerr << "current_greenlets.push_back for greenlet " << greenlet->name
              << " " << greenlet_id << std::endl;
    current_greenlets.push_back(std::move(stack_info));
  }
}

// ----------------------------------------------------------------------------
void ThreadInfo::sample(int64_t iid, PyThreadState *tstate,
                        microsecond_t delta) {
  Renderer::get().render_thread_begin(tstate, name, delta, thread_id,
                                      native_id);
  ...
  if (current_tasks.empty()) {
    ...
    for (auto &task_stack_info : current_tasks) {
      Renderer::get().render_task_begin(
          string_table.lookup(task_stack_info->task_name),
  ...
  if (!current_greenlets.empty()) {
    for (auto &greenlet_stack : current_greenlets) {
      Renderer::get().render_task_begin(
          string_table.lookup(greenlet_stack->task_name),
...
// ----------------------------------------------------------------------------
static void
for_each_thread(PyInterpreterState *interp,
  ...
  while (!threads.empty()) {
    ...
    if (tstate.next != NULL &&
        seen_threads.find(tstate.next) == seen_threads.end())
      threads.insert(tstate.next);
    if (tstate.prev != NULL &&
        seen_threads.find(tstate.prev) == seen_threads.end())
        ...
        try {
          ...
          thread_info_map.emplace(
              tstate.thread_id, std::make_unique<ThreadInfo>(
                                    tstate.thread_id, native_id, "MainThread"));
